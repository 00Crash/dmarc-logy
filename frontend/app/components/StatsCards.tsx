...
function StatCard({ label, value, helper, icon }: { label: string; value: string; helper: string; icon: ReactNode }) {
  return (
    <Card className="h-full min-h-0 shadow-none hover:shadow-sm">
      <CardContent className="grid h-full grid-cols-[minmax(0,1fr)_auto] items-center gap-2 px-3 py-2">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate text-[11px] leading-4 text-slate-500">{helper}</p>
        </div>
...
export default function StatsCards({ dashboard }: { dashboard: Dashboard }) {
  return (
    <section className="grid h-full min-h-0 grid-cols-6 gap-3">
      <StatCard label="Celkem zpráv" value={formatNumber(dashboard.total_messages)} helper="Součet count." icon={<MailCheck size={18} />} />
      ...
      <StatCard label="Zdroje" value={formatNumber(dashboard.unique_sources)} helper={`${formatNumber(dashboard.unknown_sources)} neznámé.`} icon={<Server size={18} />} />
      ...
      <StatCard label="Období" value={formatDate(dashboard.first_report_date)} helper={`Do ${formatDate(dashboard.last_report_date)}`} icon={<CalendarDays size={18} />} />
      <StatCard label="Import" value={formatDate(dashboard.first_import_date)} helper={`Do ${formatDate(dashboard.last_import_date)}`} icon={<UploadCloud size={18} />} />
    </section>
  );
}
